dofile("areas.lua")
background = gui.createcanvas(1280, 720)
...
for i=1,#area_table do
  if memory.readbyte(0x03000BF4) == area_table[i] then
    ...
    background.DrawImage("backgrounds/" .. tonumber(area_static) .. ".png", 0, 0)
    background.Refresh()
  end
end
...
if area_static ~= area_current then
  for i=1,#area_table do
    if memory.readbyte(0x03000BF4) == area_table[i] then
      background.Clear(0x000000FF)
      background.DrawImage("backgrounds/" .. tonumber(area_current) .. ".png", 0, 0)
      background.Refresh()
    end
  end
